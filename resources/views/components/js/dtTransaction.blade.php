          ...
          if (response.data.length === 0) {
            ...
            $.each(response.data, function (index, transaction) {
              ...
              if(transaction.payment_status == 'failed'){
                paymentStatus = `<span class="bg-red-100 text-red-800 text-sm font-medium me-2 px-2.5 py-0.5 rounded-sm dark:bg-red-900 dark:text-red-300">${transaction.payment_status}</span>`;
              } else if(transaction.payment_status == 'pending'){
                paymentStatus = `<span class="bg-yellow-100 text-yellow-800 text-sm font-medium me-2 px-2.5 py-0.5 rounded-sm dark:bg-yellow-900 dark:text-yellow-300">${transaction.payment_status}</span>`;
              }  else {
                paymentStatus = `<span class="bg-green-100 text-green-800 text-sm font-medium me-2 px-2.5 py-0.5 rounded-sm dark:bg-green-900 dark:text-green-300">${transaction.payment_status}</span>`;
              }

              const detailRouteBase = "{{ route('admin.transactions.show', ['transaction' => 'REPLACE_ID']) }}";
              const href = detailRouteBase.replace('REPLACE_ID', transaction.id);
